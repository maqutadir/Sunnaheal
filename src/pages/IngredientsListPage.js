// src/pages/IngredientsListPage.js
import React, { useState, useEffect } from 'react';
import { fetchAllIngredients } from '../services/apiService'; // Adjusted path
import supabase from '../supabaseClient.js'; // Adjusted path
import Icon from '../components/Icon';
import Button from '../components/Button'; // Not used, but kept for consistency if needed later
import StatusDisplay from '../components/StatusDisplay';

const IngredientsListPage = ({ navigateToDetail }) => {
    ...
    useEffect(() => {
        const loadIngredients = async () => {
            if (!supabase) {
                setError({ message: "Supabase not configured. Cannot load ingredients." });
                setIsLoading(false);
                return;
            }
            setIsLoading(true);
            setError(null);
            const { data, error: fetchError } = await fetchAllIngredients();
            if (fetchError) {
                setError(fetchError);
            } else {
                setIngredients(data.sort((a,b) => (a.name_english || "").localeCompare(b.name_english || "")));
            }
            setIsLoading(false);
        };
        loadIngredients();
    ...
    return (
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16 min-h-screen bg-beige-50">
            <header className="text-center mb-12">
                <Icon name="Leaf" className="h-16 w-16 text-amber-500 mx-auto mb-4" />
                <h1 className="text-4xl md:text-5xl font-bold text-yellow-700 mb-4">All Ingredients</h1>
                <p className="text-lg text-stone-600 max-w-2xl mx-auto">
                    Browse the complete list of Prophetic ingredients. Click on an item to view details.
                </p>
            </header>
            <StatusDisplay isLoading={isLoading} error={error} dataType="ingredients" />
            {!isLoading && !error && ingredients.length > 0 && (
             ...
             {!isLoading && !error && ingredients.length === 0 && (
                <p className="text-center text-stone-500 py-10 text-lg">No ingredients found.</p>
            )}
        </div>
    );
};
